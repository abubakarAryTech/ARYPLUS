...
const DemoSingleVideoNewV1 = memo(() => {
  const config = getConfig();
    ...
    const fetchVideoDetails = async () => {
      try {
        ...
        if (platformid === PLATFORMS.CUSTOM) {
          // alert(videoid);
          const res = await fetch(
            `${import.meta.env.VITE_APP_API_HOME_ENDPOINT}/api/cdn/ep/${videoid}`,
          );
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      try {
        ...
        await Promise.all([fetchRelatedVideos()]);
  ...
  }, [platformid, seriesid, videoid, page, navigate, location.pathname]);
...
DemoSingleVideoNewV1.displayName = "DemoSingleVideoNewV1";
export default DemoSingleVideoNewV1;
